    ...
    async createComment(data) {
        try {
            ...
            if (hasQuestion === hasAnswer) {
                throw new Error("Comment must reference either a question or an answer, not both or neither");
            }

            const [result] = await pool.execute(
                `INSERT INTO Comment (body, user_id, question_id, answer_id)
                 VALUES (?, ?, ?, ?)`,
                [data.body, data.user_id, data.question_id || null, data.answer_id || null]
            );
    ...
    async deleteComment(data) {
        try {
            ...
            if (result.affectedRows === 0) {
                return { success: false, message: "Comment not found or not authorized to delete" };
            }
